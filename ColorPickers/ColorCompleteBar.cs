...
{
	/// <summary>
	/// Description of ColorCompleteBar.
	/// </summary>
		public class FullColorBar : System.Windows.Forms.UserControl
	{
		...
		{
			int section= left*8/(this.Width);
			int step=255*8/(this.Width);
			left=left  %  ( this.Width/8);
			...
			{		
				    ...
				    case 6:return Color.FromArgb(255-left*step,0,0 );
		...
		{  int R=255;
		   ...
		   int B=0;
		   //orizzontal...
		   int colorstep=255/ ( this.Width/8);
		   int i=0;
		   //the color step x pixel
			...
			for(R=0;R<256;R +=colorstep,i++)
			{
				e.Graphics.DrawLine(new Pen(new SolidBrush(Color.FromArgb(255,255,255)),1),i,0,i,this.Height);
