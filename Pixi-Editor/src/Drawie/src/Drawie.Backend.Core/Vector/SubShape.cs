...
public class SubShape
{
    ...
    public VectorPath ToPath(VectorPath? appendTo = null)
    {
        VectorPath newPath = appendTo ?? new VectorPath();
        ...
        AddVerbToPath(CreateMoveToVerb(this), newPath);
        for (int i = 0; i < Points.Count; i++)
        {
            AddVerbToPath(Points[i].Verb, newPath);
        }
