...
namespace Drawie.Backend.Core.ColorsImpl
{
    ...
    public readonly struct Color : IEquatable<Color>
    {
        ...
        public Color(byte red, byte green, byte blue, byte alpha) => this._colorValue =
            (uint)((int)alpha << 24 | (int)red << 16 | (int)green << 8) | (uint)blue;

        public Color(byte red, byte green, byte blue) =>
            this._colorValue = (uint)(-16777216 | (int)red << 16 | (int)green << 8) | (uint)blue;
        ...
        public static Color FromHsl(float h, float s, float l, byte a = 255)
        {
            ...
            return new Color((byte)(ColorF.R * (float)byte.MaxValue), (byte)(ColorF.G * (float)byte.MaxValue),
                (byte)(ColorF.B * (float)byte.MaxValue), a);
        }

        public static Color FromHsla(float h, float s, float l, byte a)
        {
        ...
        public void ToHsl(out float h, out float s, out float l) => new ColorF((float)this.R / (float)byte.MaxValue,
            (float)this.G / (float)byte.MaxValue, (float)this.B / (float)byte.MaxValue).ToHsl(out h, out s, out l);
        ...
        public void ToHsv(out float h, out float s, out float v) => new ColorF((float)this.R / (float)byte.MaxValue,
            (float)this.G / (float)byte.MaxValue, (float)this.B / (float)byte.MaxValue).ToHsv(out h, out s, out v);
        ...
        public static bool TryParse(string hexString, out Color color)
        {
            ...
            switch (length)
            {
                ...
                case 4:
                    ...
                    if (length == 4)
                    {
                        if (!byte.TryParse(hexString[length - 4].ToString() + (object)hexString[length - 4],
                                NumberStyles.HexNumber, (IFormatProvider)CultureInfo.InvariantCulture, out result1))
                        {
                            color = Color.Empty;
                            return false;
                        }
                    ...
                    if (!byte.TryParse(hexString[length - 3].ToString() + (object)hexString[length - 3],
                            NumberStyles.HexNumber, (IFormatProvider)CultureInfo.InvariantCulture, out var result2) ||
                        !byte.TryParse(hexString[length - 2].ToString() + (object)hexString[length - 2],
                            NumberStyles.HexNumber, (IFormatProvider)CultureInfo.InvariantCulture, out var result3) ||
                        !byte.TryParse(hexString[length - 1].ToString() + (object)hexString[length - 1],
                            NumberStyles.HexNumber, (IFormatProvider)CultureInfo.InvariantCulture, out var result4))
                    {
                        color = Color.Empty;
                        return false;
                    }
                ...
                case 6:
                case 8:
                    if (!uint.TryParse(hexString, NumberStyles.HexNumber, (IFormatProvider)CultureInfo.InvariantCulture,
                            out var result5))
                    {
                        color = Color.Empty;
                        return false;
